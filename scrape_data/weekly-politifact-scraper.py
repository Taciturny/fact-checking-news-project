    ...
    for page in range(1, num_pages + 1):
        ...
        for article in articles:
            try:
                ...
                print(f"Scraped article: {claim[:50]}...")
                time.sleep(random.uniform(1, 2))
            except Exception as e:
                print(f"Error processing an article: {e}")
...
@flow(name="politifact_scraper", task_runner=SequentialTaskRunner())
def main_flow():
    logging.info("Starting PolitiFact scraper flow")
    base_url = 'https://www.politifact.com/factchecks/list/'
    num_pages = 1
    ...
    if new_fact_checks:
        num_saved = save_to_csv(new_fact_checks, 'new_df.csv')
        logging.info(f"Scraped and saved {num_saved} fact checks to new_df.csv")
        print(f"Scraped and saved {num_saved} fact checks to new_df.csv")
    else:
        print("No fact checks found.")
        logging.info("No fact checks found.")
